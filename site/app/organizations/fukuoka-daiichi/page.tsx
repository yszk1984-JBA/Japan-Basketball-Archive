...
export default function OrganizationPage() {
  return (
    <main className="detail-shell">
      ...
      <section className="roster-section"><div className="section-heading"><div><p className="eyebrow">2020 pilot</p><h2>掲載人物</h2></div><span>{players.length} records</span></div><div className="roster-list">{players.map((player) => <a href={`/players/${player.slug}`} key={player.id}><span className="number">{player.jerseyNumber}</span><div><strong>{player.name}</strong><p>{player.position} · {player.id}</p></div><ArrowRight size={19} /></a>)}</div></section>
