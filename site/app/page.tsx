  ...
  return (
    <main>
      <header className="site-header">
        <Link href="/" className="brand" aria-label="Japan Basketball Archive ホーム">
      ...
      <section className="content-section">
        <div className="section-heading"><div><p className="eyebrow">Pilot organization</p><h2>福岡第一高等学校</h2></div><Link href="/organizations/fukuoka-daiichi" className="text-link">学校ページを見る <ArrowRight size={17} /></Link></div>
        <div className="player-grid">{players.map((player, index) => <Link href={`/players/${player.slug}`} className="player-card" key={player.id}><span className="card-index">0{index + 1}</span><div><p className="card-meta">No. {player.jerseyNumber}</p><h3>{player.name}</h3><p>{player.position}</p></div><ArrowRight size={20} /></Link>)}</div>
      </section>
