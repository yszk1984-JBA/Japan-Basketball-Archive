# Batch 25 Wave 01 (帝京長岡高等学校（第2弾 学校7/12）) VERIFIED

作成日：2026-09-26

状態：VERIFIED候補、HUMAN APPROVAL・MASTER未実施

Batch 25 Wave 01 (帝京長岡高等学校（第2弾 学校7/12）)のREADY 8判断について、`eligible_fields`に一致するSUPPORTED Evidenceだけを抽出した。PARTIAL Evidence、`held_fields`、7件のHOLD Issueは除外・分離している。

このディレクトリは確認済み候補であり、正式なMaster Dataでも公開承認済みデータでもない。

## ファイル

- `person_verified.csv`：2人
- `organization_verified.csv`：5組織
- `career_verified.csv`：6件
