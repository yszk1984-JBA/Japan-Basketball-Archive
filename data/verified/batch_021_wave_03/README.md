# Batch 21 Wave 03 (中部大学第一高等学校（第2弾 学校3/12）) VERIFIED

作成日：2026-09-26

状態：VERIFIED候補、HUMAN APPROVAL・MASTER未実施

Batch 21 Wave 03 (中部大学第一高等学校（第2弾 学校3/12）)のREADY 12判断について、`eligible_fields`に一致するSUPPORTED Evidenceだけを抽出した。PARTIAL Evidence、`held_fields`、8件のHOLD Issueは除外・分離している。

このディレクトリは確認済み候補であり、正式なMaster Dataでも公開承認済みデータでもない。

## ファイル

- `person_verified.csv`：3人
- `organization_verified.csv`：7組織
- `career_verified.csv`：9件
...
- `issue_dispositions.csv`：8件のHOLD
