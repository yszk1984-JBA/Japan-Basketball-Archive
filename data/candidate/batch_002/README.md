...
| `organization_candidates.csv` | 今回のCareerが参照する19組織 |
| `career_candidates.csv` | 高校・大学・留学・プロのCareer候補30件 |
...
| `exception_queue.csv` | 新しい公式資料が見つかるまで保留する19件の作業表 |
| `exception_queue_summary.md` | 例外キューの件数、運用ルール、再開条件 |
...
python3 scripts/build_batch_002_exception_queue.py
```
...
未解決19件は例外キューへ分離した。同じ検索は繰り返さず、新しい公式資料が得られた時だけ再開する。例外キューが残っていても、確認済み項目のレビューと次バッチの人数拡大は進められる。
