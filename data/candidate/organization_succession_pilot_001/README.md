...
## 対象外とした1件（2026-09-23、Yuichiと合意し保留と決定）
...
Yuichiの判断：Organizationは、実際にCareerが必要とした時点でのみ起票する（従来からの運用を維持）。承継チェーンを完全にするためだけにOrganizationを先回りで登録することはしない。そのため、この事例は承継リンクを作成できないまま保留とする。将来、サンロッカーズ渋谷名義のCareerが実際に必要になった時点で、Organization登録とあわせて承継リンクも作成する。
