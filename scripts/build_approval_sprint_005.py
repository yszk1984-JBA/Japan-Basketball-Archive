#!/usr/bin/env python3
"""Build the Human Approval packet for Batch 006 Wave 2 VERIFIED candidates."""
...
SOURCE = ROOT / "data" / "verified" / "batch_006_wave_02"
OUTPUT = ROOT / "data" / "verified" / "approval_sprint_005"
...
def main() -> None:
    ...
    for person in people:
        ...
        person_review.append({
            ...
            "name": person["name"],
            "verified_batch": "Batch 006 Wave 2",
    ...
    organization_review = [{"verified_batch": "Batch 006 Wave 2", **row} for row in organizations]
    evidence_review = [{"verified_batch": "Batch 006 Wave 2", **row} for row in evidence]
    source_review = [{"verified_batch": "Batch 006 Wave 2", **row} for row in sources]
    held_fields_review = [{"verified_batch": "Batch 006 Wave 2", **row} for row in held_fields]
    ...
    errors: list[str] = []
    expected = {
        "people": (len(people), 4), "careers": (len(careers), 12),
        "organizations": (len(organizations), 7), "evidence": (len(evidence), 57),
        "sources": (len(sources), 13), "issues": (len(issues), 10),
    ...
    validation = [
        "# Approval Sprint 005 検証レポート", "", f"作成日：{CREATED_AT}", "",
    ...
    if errors:
        raise SystemExit("Approval Sprint 005 validation failed")

    names = "、".join(row["name"] for row in person_review)
    readme = f"""# Approval Sprint 005
...
Batch 006 Wave 2でVERIFIED候補となり、まだMasterに含まれていない4名を対象とする。

{names}

## 対象版

- Batch 006 Wave 2 VERIFIED：`{VERIFIED_COMMIT}`
...
"""
    ...
    summary = [
        "# Approval Sprint 005 レビュー要約", "", f"作成日：{CREATED_AT}", "",
...
> 対象：Approval Sprint 005の4名（{names}）。対象版：Batch 006 Wave 2 VERIFIED `{VERIFIED_COMMIT}`。範囲：`evidence_review.csv`のSUPPORTED Evidenceに対応するPerson・Career・Organization。判断：対象範囲をMasterへ反映してよい。`hold_review.csv`の全{len(hold_review)}件と`held_fields_review.csv`の全{len(held_fields_review)}件は承認対象外。
"""
    (OUTPUT / "approval_request.md").write_text(request, encoding="utf-8")

    print(
        f"Approval Sprint 005: {len(person_review)} persons, {len(career_review)} careers, "
