#!/usr/bin/env python3
"""Validate Batch 007 Wave 4b (深掘りWave追補) candidate structure.

This addendum wave references Career IDs from Wave 1 (C000299) and Wave 4
(via source B7S0026), and adds one brand-new Career (C000330, junior high
school). Entity references are checked against the union of all
batch_007 waves' candidates (including this one) plus Master.
"""
...
def main() -> int:
    ...
    # This addendum's decisions must not silently regress a prior wave's decision
    # for the same entity to a WEAKER state without new reasoning; here they are
    # both upgrades (HOLD -> READY / new READY), which is allowed.
    ...
    report = [
        "# Batch 007 Wave 8（深掘りWave）検証レポート", "", "作成日：2026-09-23", "",
        ...
        "- 対象：辻直人（P000097）のプロキャリア全歴（東芝ブレイブサンダース→川崎ブレイブサンダース→広島ドラゴンフライズ→群馬クレインサンダーズ）",
        "- Wave1〜4の既存ファイルは未変更（追記のみ）", "- VERIFIED・Master・公開サイト：未変更", "", "## エラー", "",
    ]
