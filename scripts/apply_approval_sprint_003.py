#!/usr/bin/env python3
"""Apply Yuichi's explicit Approval Sprint 003 decision to MASTER."""
...
PACKET = ROOT / "data" / "verified" / "approval_sprint_003"
MASTER = ROOT / "data" / "master"
PACKET_COMMIT = "a25eed93913e7bc40d43fd5cc672304a48a840cd"
...
def verify_approval() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("Approval Sprint 003 differs from the approved review packet")
    approval = (PACKET / "master_approval.md").read_text(encoding="utf-8")
    required = [APPROVAL_ID, PACKET_COMMIT, "承認します", "承認者：Yuichi"]
    if any(value not in approval for value in required):
        raise SystemExit("Approval Sprint 003 approval record is incomplete")
...
- `{APPROVAL_ID}`：Approval Sprint 003
