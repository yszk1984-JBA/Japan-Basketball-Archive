...
APPROVAL_SPRINT = ROOT / "data" / "verified" / "approval_sprint_001" / "person_review.csv"
...
def main() -> None:
    ...
    sprint_approval_id = "APP-AS001-20260921-01"
    batch_005_approval_id = "APP-B005-20260921-01"
    for approval_id in (sprint_approval_id, batch_005_approval_id):
        if approval_id not in approvals_by_id:
            raise SystemExit(f"Missing Master approval record: {approval_id}")
    with APPROVAL_SPRINT.open(encoding="utf-8-sig", newline="") as handle:
        sprint_person_ids = {row["person_id"] for row in csv.DictReader(handle)}
    ...
    for person in people:
        ...
        public_people.append({
            ...
            "dataStatus": "master",
            "approvalId": sprint_approval_id if person_id in sprint_person_ids else batch_005_approval_id,
            "facts": facts,
    ...
    approval = approvals_by_id[sprint_approval_id]
