#!/usr/bin/env python3
"""Validate Batch 005 VERIFIED candidates without treating them as MASTER."""
...
BASE = Path("data/verified/batch_005")
SCOPE = Path("data/candidate/batch_005/review_packet/verification_scope.csv")
...
def main() -> int:
    ...
    expected_counts = {
        ...
        "organizations": (len(organizations), 11),
        "careers": (len(careers), 11),
        "sources": (len(sources), 15),
        "evidence": (len(evidence), 84),
        "issues": (len(issues), 9),
        "decisions": (len(decisions), 26),
    }
    ...
    report = [
        "# Batch 005 VERIFIED検証レポート", "", "作成日：2026-09-21", "",
        "## 結果", "",
