#!/usr/bin/env python3
"""Build the consolidated Human Approval packet for Batch 019〜030
(強豪校横展開 第2弾、学校1〜12、18 waves、46名) VERIFIED candidates.

Copied from build_approval_sprint_006.py (same traditional, Tier-less
method as Sprints 001-006) with a new MANIFEST and expected counts.

This generalizes build_approval_sprint_005.py (single-wave) to loop over
every wave promoted by build_verified_batches_019_030.py and produce ONE
...
OUTPUT = ROOT / "data" / "verified" / "approval_sprint_007"
VERIFIED_COMMIT = "adf7443"
CREATED_AT = "2026-09-26"

# (batch, wave, school label) -- mirrors build_verified_batches_019_030.py's
# MANIFEST (commit column dropped; not needed here since all waves share
# the same VERIFIED_COMMIT once promoted).
MANIFEST = [
    ("019", "wave_01", "桐光学園高等学校（第2弾 学校1/12）"),
    ("019", "wave_02", "桐光学園高等学校（第2弾 学校1/12）"),
    ("020", "wave_01", "船橋市立船橋高等学校（第2弾 学校2/12）"),
    ("020", "wave_02", "船橋市立船橋高等学校（第2弾 学校2/12）"),
    ("021", "wave_01", "中部大学第一高等学校（第2弾 学校3/12）"),
    ("021", "wave_02", "中部大学第一高等学校（第2弾 学校3/12）"),
    ("021", "wave_03", "中部大学第一高等学校（第2弾 学校3/12）"),
    ("022", "wave_01", "尽誠学園高等学校（第2弾 学校4/12）"),
    ("022", "wave_02", "尽誠学園高等学校（第2弾 学校4/12）"),
    ("023", "wave_01", "前橋育英高等学校（第2弾 学校5/12）"),
    ("024", "wave_01", "正智深谷高等学校（第2弾 学校6/12）"),
    ("025", "wave_01", "帝京長岡高等学校（第2弾 学校7/12）"),
    ("026", "wave_01", "桜丘高等学校（第2弾 学校8/12）"),
    ("027", "wave_01", "秋田県立能代工業高等学校（第2弾 学校9/12）"),
    ("027", "wave_02", "秋田県立能代工業高等学校（第2弾 学校9/12）"),
    ("028", "wave_01", "報徳学園高等学校（第2弾 学校10/12）"),
    ("029", "wave_01", "大阪桐蔭高等学校（第2弾 学校11/12）"),
    ("030", "wave_01", "近畿大学附属高等学校（第2弾 学校12/12）"),
]
...
def unchanged_since_verified() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("Batch 019-030 VERIFIED snapshot changed after promotion")
...
def main() -> None:
    ...
    for person in all_people:
        ...
        person_review.append({
            ...
            "priority_reason": "強豪校横展開 第2弾（学校1〜12）：出身校を起点とするCareer",
        })
    ...
    errors: list[str] = []
    expected = {
        "people": (len(all_people), 46),
        "careers": (len(all_careers), 137),
        "organizations (raw, pre-dedup)": (len(all_organizations), 105),
        "organizations (unique)": (len(organization_review), len(organization_by_id)),
        "evidence": (len(evidence_review), 274),
        "sources": (len(source_review), 46),
        "issues": (len(hold_review), 141),
        "held fields": (len(held_fields_review), 137),
    }
    ...
    validation = [
        "# Approval Sprint 007 検証レポート", "", f"作成日：{CREATED_AT}", "",
    ...
    if errors:
        raise SystemExit("Approval Sprint 007 validation failed")
    ...
    evidence_entities = {row["entity_id"] for row in evidence_review}
    no_evidence_careers = [row for row in career_review if row["career_id"] not in evidence_entities]
    special_lines = "\n".join(
        f"- {row['career_id']}：{row['name']} → {row['organization_name']}（SUPPORTED Evidenceなし。公式サイトの表記が文字化けしており、Yuichiの判断（2026-09-26）で白鷗大学として登録した項目。evidenceはPARTIALのためVERIFIED Evidenceには含まれない）"
        for row in no_evidence_careers
    ) or "- なし"

    readme = f"""# Approval Sprint 007
...
"""
    ...
    summary_lines = [
        "# Approval Sprint 007 レビュー要約", "", f"作成日：{CREATED_AT}", "",
        "## 学校別内訳", "",
...
> 対象：Approval Sprint 007の{len(person_review)}名（強豪校横展開 第2弾、学校1〜12）。対象版：VERIFIED snapshot `{VERIFIED_COMMIT}`。範囲：`evidence_review.csv`のSUPPORTED Evidenceに対応するPerson・Career・Organization。判断：対象範囲をMasterへ反映してよい。`hold_review.csv`の全{len(hold_review)}件と`held_fields_review.csv`の全{len(held_fields_review)}件は承認対象外。個別判断項目（{", ".join(r["career_id"] for r in no_evidence_careers) or "なし"}）は含める／除外する、のどちらかを明記する。
...
"""
    ...
    print(
        f"Approval Sprint 007: {len(person_review)} persons, {len(career_review)} careers, "
