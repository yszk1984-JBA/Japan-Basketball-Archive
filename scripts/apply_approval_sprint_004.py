#!/usr/bin/env python3
"""Apply Yuichi's explicit Approval Sprint 004 decision to MASTER."""
...
PACKET = ROOT / "data" / "verified" / "approval_sprint_004"
MASTER = ROOT / "data" / "master"
PACKET_COMMIT = "df85da5a3c302ce674cb6062565aa83b527a0fa2"
VERIFIED_COMMIT = "865415a1ab6f0008130bbb2dac1ae9f9d98759e1"
...
def verify_approval() -> None:
    ...
    if result.returncode != 0:
        raise SystemExit("Approval Sprint 004 differs from the approved review packet")
    approval = (PACKET / "master_approval.md").read_text(encoding="utf-8")
    required = [
        APPROVAL_ID,
        PACKET_COMMIT,
        VERIFIED_COMMIT,
        "Approval Sprint 004承認します",
        "承認者：Yuichi",
    ]
    if any(value not in approval for value in required):
        raise SystemExit("Approval Sprint 004 approval record is incomplete")
...
def main() -> None:
    ...
    master_approvals = merge(
        ...
        [{
            ...
            "approval_reference": "data/verified/approval_sprint_004/master_approval.md",
        }],
...
- `{APPROVAL_ID}`：Approval Sprint 004
