#!/usr/bin/env python3
"""Create Batch 006 Wave 2 VERIFIED candidates from READY decisions only.
...
WAVE = ROOT / "data" / "candidate" / "batch_006" / "wave_02"
OUTPUT = ROOT / "data" / "verified" / "batch_006_wave_02"
SOURCE_COMMIT = "62112f1"
...
def main() -> None:
    ...
    if len(ready) != 23 or len(holds) != 0:
    ...
    scope = f"""# Batch 006 Wave 2 VERIFIED候補 作成範囲

作成日：{CREATED_AT}

Batch 006 Wave 2のQA判断から、READY 23件の`eligible_fields`だけをVERIFIED候補へ抽出した。

- CANDIDATE・QA基準commit：`{SOURCE_COMMIT}`
- 対象：`data/candidate/batch_006/wave_02/qa_decisions.csv`のREADY 23件
...
"""
    ...
    report = f"""# Batch 006 Wave 2 VERIFIED生成レポート
...
"""
    ...
    readme = f"""# Batch 006 Wave 2 VERIFIED

作成日：{CREATED_AT}

状態：VERIFIED候補、HUMAN APPROVAL・MASTER未実施

Batch 006 Wave 2のREADY 23判断について、`eligible_fields`に一致するSUPPORTED Evidenceだけを抽出した。PARTIAL Evidence、`held_fields`、10件のHOLD Issueは除外・分離している。
