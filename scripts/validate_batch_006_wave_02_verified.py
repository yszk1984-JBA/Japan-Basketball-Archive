#!/usr/bin/env python3
"""Validate Batch 006 Wave 2 VERIFIED candidates without treating them as MASTER."""
...
BASE = ROOT / "data" / "verified" / "batch_006_wave_02"
SCOPE = ROOT / "data" / "candidate" / "batch_006" / "wave_02" / "qa_decisions.csv"
...
def main() -> int:
    ...
    expected = {
        ...
        "organizations": (len(organizations), 7),
        "careers": (len(careers), 12),
        "sources": (len(sources), 13),
        "evidence": (len(evidence), 57),
        "held": (len(held), 10),
        "issues": (len(issues), 10),
        "ready decisions": (len(ready), 23),
    }
    ...
    report = [
        "# Batch 006 Wave 2 VERIFIED検証レポート", "", "作成日：2026-09-22", "",
