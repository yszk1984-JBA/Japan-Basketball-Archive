#!/usr/bin/env python3
"""Build a beginner-friendly review packet for Batch 003.
...
BATCH_DIR = Path("data/candidate/batch_003")
...
FIELD_LABELS = {
    ...
    "activity_date": "活動日",
    "activity_year": "活動確認年",
    "competition_participation": "競技参加",
...
def main() -> None:
    ...
    lines = [
        ...
        "## 重要な注意点",
        "",
        "- 岩下周介の2023年学年はJUBF公式の2年を候補とし、外部回答の4年は採用していない。",
        "- カマレ ムレマ フランシスの大学背番号はJUBF公式の70を候補とし、外部回答の75は採用していない。",
        "- 當山修梧はJUBF公式の2021年・2022年がともに2年表示のため、学年を確認対象から外している。",
        "- 本松龍斗は日本体育大学のPlayerではなく、JUBF公式PROFILEに基づくStudent Coachとしている。高校での役割は未確認のため高校Careerを作成していない。",
        "- 鷹野祐磨は卒業後の実所属を確認できず、大学Careerを作成していない。",
