...
パス内に`%3D`を含む10件は、2026-09-18の確認で公開ページへ到達し、タイトルも一致した。この表記だけを理由に修正しない。`SRC000023`は初回取得時に502となったが、2026-09-19の再確認でタイトルと本文が一致した。詳細は[PILOT_SOURCE_URL_REVIEW](PILOT_SOURCE_URL_REVIEW.md)に記録した。
...
5. `P000027`と項目別出典不足をBatch 001で検証し、未解決事項を分離してから次の範囲へ進める。
