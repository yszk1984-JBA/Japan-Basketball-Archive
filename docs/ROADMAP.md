...
| 2：候補と検証 | 合意した小規模範囲で取込と検証を実装 | 進行中。Batch 001のCANDIDATE、項目別出典、未解決事項、Python構造検査を作成 |
...
現在は段階2の候補と検証を進めている。公開画面を早期に検証するため、確認中の少数データを使うWebプロトタイプを並行して作成する。プロトタイプの表示データはVERIFIEDまたはMASTERを意味しない。data/verifiedとdata/masterは必要になる段階まで作成しない。
